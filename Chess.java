...
public class Chess {
			...
			if (checkPieceColor(board.get(from)) == turn) {
				...
				if (currentPiece.move(from, to)) {
					if (pawnPromotion) {
						...
						pawnPromotion = false;
						ReturnPiece pawn = board.get(to);
						if (parts.length >=3) {
							currentPiece.pawnPromotion(parts[2], pawn);

						} else {
							currentPiece.pawnPromotion(null, pawn);
						}
					}
	...
	public static void start() {
		...
		whiteRook1.pieceRank = 7;
		whiteRook1.pieceFile = PieceFile.b;
